...
import com.kotlin.baselibrary.adapter.ViewPagerAdapter;
...
public class EasyNavigationBar extends LinearLayout {
    ...
    private void toDp() {
        ...
        msgPointTextSize = ConvertUtils.sp2px(msgPointTextSize);

        tabTextTop = ConvertUtils.dp2px(tabTextTop);
        tabTextSize = ConvertUtils.sp2px(tabTextSize);
        ...
        addTextSize = ConvertUtils.sp2px(addTextSize);
...
//若没有设置中间添加的文字字体大小、颜色、则同其他Tab一样
    ...
    public void buildNavigation() {
        if ((titleItems.length != normalIconItems.length) || (titleItems.length != selectIconItems.length) || (normalIconItems.length != selectIconItems.length)) {
            LogUtils.e("EasyNavigation", "请传入相同数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
    ...
    public void buildAddNavigation() {
        ...
        tabCount = titleItems.length;
        if (tabCount % 2 == 0) {
            LogUtils.e("EasyNavigation", "MODE_ADD模式下请传入奇奇奇奇奇奇奇奇奇奇奇数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }

        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                LogUtils.e("EasyNavigation", "MODE_ADD模式下/addAsFragment=true时Fragment的数量应和传入tab集合数量相等");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                LogUtils.e("EasyNavigation", "MODE_ADD模式下/addAsFragment=false时Fragment的数量应比传入tab集合数量少一个");
                return;
            }
        }
    ...
    public void buildAddViewNavigation() {
        ...
        if (tabCount % 2 == 0) {
            LogUtils.e("EasyNavigation", "MODE_ADD_VIEW模式下请传入偶偶偶偶偶偶偶偶偶偶偶偶数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                LogUtils.e("EasyNavigation", "MODE_ADD_VIEW模式下/addAsFragment=true时Fragment的数量应比传入tab集合数量多一个");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                LogUtils.e("EasyNavigation", "MODE_ADD_VIEW模式下/addAsFragment=false时,Fragment的数量应和传入tab集合数量相等");
                return;
            }
        }
    ...
    public EasyNavigationBar addLayoutHeight(int addLayoutHeight) {
        this.addLayoutHeight = ConvertUtils.dp2px(addLayoutHeight);
        return this;
    }
